...
async function uploadAndInsert() {
  console.log('=== UPLOADING BUILD 53 (64-bit arm64-v8a) TO SUPABASE ===');
  
  const apkPath = path.join(__dirname, '..', 'FarmaCompareV-1.0.0+53.apk');
  const bucketName = 'apk';
  const remoteFileName = 'farmacompare-v1.0.0-build53.apk';
    ...
    .insert([
      {
        ...
        version_name: '1.0.0',
        apk_url: publicUrl,
        release_notes: 'v1.0.0 (Build 53): Ajuste de escala al 85% para el logotipo en el icono de la aplicación (launcher icon), solucionando problemas de visualización recortada en la pantalla de inicio del teléfono. Mejoras visuales y optimizaciones generales.',
        force_update: false
      }
    ])
    .select();
