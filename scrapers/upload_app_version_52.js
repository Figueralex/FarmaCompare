...
async function uploadAndInsert() {
  console.log('=== UPLOADING BUILD 52 (64-bit arm64-v8a) TO SUPABASE ===');
  
  const apkPath = path.join(__dirname, '..', 'FarmaCompareV-1.0.0+52.apk');
  const bucketName = 'apk';
  const remoteFileName = 'farmacompare-v1.0.0-build52.apk';
    ...
    .insert([
      {
        ...
        version_name: '1.0.0',
        apk_url: publicUrl,
        release_notes: 'v1.0.0 (Build 52): Renovación completa de identidad. Integración de tu nuevo logotipo oficial tanto en el icono del menú de inicio de tu celular (launcher icon) como en las pantallas de cabecera de la aplicación y simuladores.',
        force_update: false
      }
    ])
    .select();
