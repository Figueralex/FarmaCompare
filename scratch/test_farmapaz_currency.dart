...
void main() async {
  ...
  // 1. Petición con wmc-currency=USD
  try {
    print("=== PETICION CON wmc-currency=USD ===");
    final resp = await dio.get(
      'https://farmapazvenezuela.com/?s=LA+SANTE+500&post_type=product&wmc-currency=USD',
      options: Options(headers: headers),
    );
    _parseResults(resp.data.toString());
  } catch(e) {
    print("Error USD: $e");
  }

  // 2. Petición sin wmc-currency=USD (por defecto)
  try {
    print("\n=== PETICION SIN wmc-currency=USD (Voz por defecto) ===");
    final resp = await dio.get(
      'https://farmapazvenezuela.com/?s=LA+SANTE+500&post_type=product',
      options: Options(headers: headers),
    );
    _parseResults(resp.data.toString());
  } catch(e) {
    print("Error por defecto: $e");
  }
}
